...
enum SLLidarState {
  ...
  CONNECTED_WARN,
  CONNECTED_ERR,
  // CONNECTED_NO_READINGS
};

bool need_exit = false;

class SLLidarNodeStable : public rclcpp::Node {
 public:
  SLLidarNodeStable() : Node("sllidar_node") {
    ...
    // as stop_motor() / start_motor() check whether lidar is connected,
    // services can run anytime
    stop_motor_service = this->create_service<std_srvs::srv::Empty>(
        "stop_motor", std::bind(&SLLidarNodeStable::stop_motor, this,
                                std::placeholders::_1, std::placeholders::_2));
    start_motor_service = this->create_service<std_srvs::srv::Empty>(
        "start_motor", std::bind(&SLLidarNodeStable::start_motor, this,
                                 std::placeholders::_1, std::placeholders::_2));
  }
 ...
 private:
  void init_param() {
    ...
    this->declare_parameter<std::string>("hardware_id", "lidar");
    ...
    this->get_parameter_or<std::string>("hardware_id", hardware_id, "lidar");
  ...
  SLLidarState checkSLLIDARHealth(ILidarDriver* drv) {
    sl_result op_result;
      ...
      switch (healthinfo.status) {
        ...
        case SL_LIDAR_STATUS_ERROR:
          RCLCPP_ERROR(this->get_logger(),
                       "Error, SLLidar internal error detected. Please reboot "
                       "the device to retry.");
          return CONNECTED_ERR;
        default:
          RCLCPP_ERROR(this->get_logger(),
                       "Error, Unknown internal error detected. Please reboot "
                       "the device to retry.");
          return CONNECTED_ERR;
      }
    } else {
  ...
  bool stop_motor(const std::shared_ptr<std_srvs::srv::Empty::Request> req,
    ...
    // if (!drv) return false;
    if (!((state == CONNECTED) || (state == CONNECTED_WARN))) return false;
  ...
  bool config_and_start() {
    ...
    if (scan_mode.empty()) {
      ...
      if (SL_IS_OK(op_result)) {
        ...
        if (selectedScanMode == sl_u16(-1)) {
          ...
          op_result =
              drv->startScanExpress(false /* not force scan */,
    ...
    if (SL_IS_OK(op_result)) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "current scan mode: %s, sample rate: %d Khz, max_distance: "
                  "%.1f m, scan frequency:%.1f Hz, ",
    ...
    if (op_result == SL_RESULT_OK) {
      ...
      if (op_result == SL_RESULT_OK) {
        if (angle_compensate) {
          ...
          // if readings are back change to default("ok") state
          // if (state == CONNECTED_NO_READINGS) state = CONNECTED_ENABLED;
          return true;
        }
      } else if (op_result == SL_RESULT_OPERATION_FAIL) {  // ascendScanData
        ...
        // if (state == CONNECTED_ENABLED) state = CONNECTED_NO_READINGS;
      }
    } else {  // if grabScanDataHq != OK
      // if (state == CONNECTED_ENABLED) state = CONNECTED_NO_READINGS;
    }
    return false;
  }
  ...
  void publish_status() {
    ...
    switch(state) {
      ...
      case CONNECTED_WARN:
        msg->level = diagnostic_msgs::msg::DiagnosticStatus::WARN;
        msg->message = "Internal warning";
        break;
      case CONNECTED_ERR:
        msg->level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
        msg->message = "Internal error, try reconnecting";
        break;
      default:
        break;
    }
 ...
 public:
  int work_loop() {
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "SLLidar running on ROS2 package SLLidar. SLLIDAR SDK Version:%d.%d.%d",
...
int main(int argc, char* argv[]) {
  ...
  auto sllidar_node = std::make_shared<SLLidarNodeStable>();
