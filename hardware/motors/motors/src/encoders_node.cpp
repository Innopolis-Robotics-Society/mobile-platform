...
class EncodersNode : public rclcpp::Node
{
  public:
	...
	positions = {0, 0};
        ...
        timer_ = this->create_wall_timer(PERIOD, std::bind(&EncodersNode::send_request, this));
    }
  ...
  private:
	...
	if (msg.data[1] == 0x6C) {
		overlord100_msgs::msg::WheelsData vels;
	...
	positions[msg.data[3]-1] = static_cast<int32_t>(msg.data[4]+(msg.data[5]<<8)+(msg.data[6]<<16)+(msg.data[7]<<24));

	overlord100_msgs::msg::WheelsData angles;
	angles.left = positions[0];
	angles.right = positions[1];
