...
class ModeChangeClient : public rclcpp::Node {
   public:
    ModeChangeClient() : Node("mode_change_client") {
        client_ = this->create_client<overlord100_msgs::srv::ChangeMode>(
            "change_mode");
    ...
    bool send_request(int mode) {
        auto request =
            std::make_shared<overlord100_msgs::srv::ChangeMode::Request>();
        request->mode = mode;
        ...
        if (rclcpp::spin_until_future_complete(this->shared_from_this(),
            ...
            RCLCPP_ERROR(this->get_logger(),
                         "Failed to call service change_mode");
            return false;
        }
    }

   private:
    rclcpp::Client<overlord100_msgs::srv::ChangeMode>::SharedPtr client_;
};
