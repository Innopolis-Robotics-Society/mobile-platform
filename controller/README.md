...
- iros_mobile_platform_controller that recalculates the velocity of the platform's center into wheels velocities,

- iros_mobile_platform_bringup contains the core file for launching the whole software stack for the robot.

For launching on the hardware use this command:

```ros2 launch iros_mobile_platform_bringup hardware_control.launch.py```

For launching on with simulation use this command:

```ros2 launch iros_mobile_platform_bringup simulation_control.launch.py```
