# iros_mobile_platform-controller
...
*cmd_vel_publisher (`cmd_vel_publisher.cpp`) sends `Twist` over the topic `cmd_vel` to the iros_mobile_platform_controller (`controller.cpp`). And the controller sends `WheelData` to the controller_subscriber (`controller_subscriber.cpp`) over the topic `wheels_control`*
...
- `/wheels_control` (`iros_mobile_platform_msgs/msg/WheelsData`): Publishes the calculated wheel velocities to control the robot. 
...
```ros2 run iros_mobile_platform_controller cmd_vel_publisher```
```ros2 run iros_mobile_platform_controller diff_drive_controller```
```ros2 run iros_mobile_platform_controller controller_subscriber```