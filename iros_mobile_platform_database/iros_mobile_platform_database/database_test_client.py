...
import numpy as np
from iros_mobile_platform_database_msgs.srv import (
    StoreMap,
    GetMap,
    LoadMap,
    DeleteMap,
...
from iros_mobile_platform_database_msgs.msg import Point as PointMsg, Route as RouteMsg
...
class DatabaseTestClient(Node):
    def __init__(self):
        ...
        self.db_service_prefix = "/iros_mobile_platform_database_node"
    ...
    def run_tests(self):
        ...
        # Find map files
        try:
            package_share_directory = get_package_share_directory("iros_mobile_platform")
            ...
            self.get_logger().error(
                f"Could not find iros_mobile_platform package share directory or maps. Make sure the package is installed. Error: {e}"
            )
            return
